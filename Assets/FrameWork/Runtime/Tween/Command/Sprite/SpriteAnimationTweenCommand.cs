...
{
    [System.Serializable]
    public class SpriteAnimationTweenCommand : TweenCommand
    {
        ...
        public override void Execute(ref Sequence sequence, bool isJoinMode, float timeScale = 1f, float startDelay = 0f)
        {
            ...
            Tween tween = Tween.Custom(target, 0f, 0.999f, finalDuration, delegate (SpriteRenderer t, float val)
            {
        ...
        {
            if (target == null)
                return "Sprite 애니메이션: 타겟 없음";

            string frameText = animationFrames != null
                ? $"{animationFrames.Length}프레임"
                : "0프레임";

            return $"Sprite 애니메이션: {target.name} ({frameText}, {duration:0.##}초)";
        }
        ...
        public override void Draw(Rect rect)
        {
            ...
            target = (SpriteRenderer)EditorGUI.ObjectField(new Rect(rect.x, currentY, rect.width, h), "Target", target, typeof(SpriteRenderer), true);
        ...
        public override int GetNumRows()
        {
            ...
            rows += 1; // target 단일 필드 높이 (1줄 고정으로 예측 가능해짐)
