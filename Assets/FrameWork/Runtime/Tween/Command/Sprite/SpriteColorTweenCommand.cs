...
{
    [System.Serializable]
    public class SpriteColorTweenCommand : TweenCommand, IColorTweenCommand
    {
        ...
        public override void Execute(ref Sequence sequence, bool isJoinMode, float timeScale = 1f, float startDelay = 0f)
        {
            ...
            Color finalColor = isRandomEndColor
                ? new Color(
                    Random.Range(minEndColor.r, maxEndColor.r),
                    Random.Range(minEndColor.g, maxEndColor.g),
                    Random.Range(minEndColor.b, maxEndColor.b),
                    Random.Range(minEndColor.a, maxEndColor.a))
                : minEndColor;
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                {
                    tween = Tween.Color(targets[i], finalColor, finalDuration, animationCurve, startDelay: startDelay);
                }
                else
                {
                    tween = Tween.Color(targets[i], finalColor, finalDuration, (Ease)selectedEaseOrCurve, startDelay: startDelay);
                }
        ...
        public override string GetDescription()
        {
            ...
            return $"Sprite 색상: {targetText} → {colorText} ({durationText})";
        }
        ...
        public override void Draw(Rect rect)
        {
            ...
            EditorGUI.LabelField(new Rect(rect.x, currentY, rect.width, h), "타겟 (SpriteRenderer) 설정", headerStyle);
            currentY += h + 2;
